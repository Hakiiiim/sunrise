...
if args.evaluate:
    ...
    avg_reward, avg_Q = ensemble_test(args, 0, dqn_list, val_mem, metrics, results_dir, 
                                      num_ensemble=args.num_ensemble, evaluate=True, SIZE = SIZE, NUM_TRAP = NUM_TRAP, traps=traps, goal=goal, start=start)  # Test
        ...
        if T >= args.learn_start:
            ...
            if T % args.evaluation_interval == 0:
                ...
                avg_reward, avg_Q = ensemble_test(args, T, dqn_list, val_mem, metrics, results_dir, 
                                                  num_ensemble=args.num_ensemble, SIZE = SIZE, NUM_TRAP = NUM_TRAP, traps=traps, goal=goal, start=start, iteration = T)  # Test
